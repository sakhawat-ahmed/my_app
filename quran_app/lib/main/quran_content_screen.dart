...
class QuranContentScreen extends StatelessWidget {
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            Text('Error: ${quranProvider.error}', textAlign: TextAlign.center),
            const SizedBox(height: 16),
            ElevatedButton(
      ...
      itemBuilder: (context, index) {
        ...
        return Card(
          ...
          child: ListTile(
            ...
            subtitle: Text('${surah['name']} • ${surah['englishNameTranslation']}'),
            trailing: Text('${surah['numberOfAyahs']} verses', style: const TextStyle(color: Colors.grey)),
            onTap: () => _navigateToSurahDetail(context, surah['number']),
          ),
        );
      },
    );
  ...
  void _navigateToSurahDetail(BuildContext context, int surahNumber) async {
    ...
    try {
      ...
      final surahData = await quranProvider.getSurahWithTranslation(surahNumber);
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Failed to load surah: $e'), backgroundColor: Colors.red),
      );
    }
  }
}